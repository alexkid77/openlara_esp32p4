...
*/
    ...
    #ifdef _GAPI_SW
        if (background[0]) {
            // The software rasterizer only supports 8-bit indexed textures,
            // so blit the full-RGBA title/background image directly into the
            // framebuffer (DOS-style) instead of using the mesh pipeline.
            GAPI::blitTitleBG(background[0], sx, sy, alpha);
            return;
        }
    #endif
